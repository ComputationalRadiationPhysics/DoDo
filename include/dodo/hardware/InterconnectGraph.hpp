...
namespace dodo
{
    namespace hardware
    {
                    ...
                    for (auto pos2(pos + 1); j != adjacentVertices.second; ++j, ++pos2)
                    {
                        ...
                        for (auto inE(inEdges.first); inE != inEdges.second; ++inE)
                        {
                            ...
                            for (auto outE(outEdges.first); outE != outEdges.second; ++outE)
                            {
                                ...
                                for (auto e(oldEdgesBetween.first); !hasBetterPath && e != oldEdgesBetween
                                    .second; ++e)
                                {
            ...
            void addNewEdges(
                std::list<
                    std::tuple<
                        ...
                        VertexID,
                        EdgeProperties,
                        EdgeHistory>> nEdges, EdgeHistoryMap &hMap
            )
            {
        ...
        template<
            ...
            typename T_IsoMap
        >
        auto copyAndTransform(
            std::shared_ptr<T_OldInterconnect_t> interconnectGraph,
            T_IsoMap &isoMap
        )
        {
