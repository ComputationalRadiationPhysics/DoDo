...
namespace graph
{
     ...
     *
     * When no dynamic properties are given, writes only the structural
     * information of the Graph.
     * Custom dynamic properties can be submitted to improve the
    ...
    )
    {
        ...
        detail::member_iterator< boost::vertex_bundle_t >(
            dp,
            g
        );
        detail::member_iterator< boost::edge_bundle_t >(
            dp,
            g
        );

        graph::simpleBGLWriter(
            os,
            graph,
            dp
        );
     ...
     * The function writes to an output stream the graph in the .graphml format.
     *
     * @param os    an output stream to write to
     * @param graph the BGL graph you want to print. Should inherit from
     *              SimpleBGLBase to be printable with this function
     */
    template<
        typename T_Graph,
        typename T_SFINAE = typename std::enable_if<
    ...
    /**
     * Adapter for operator<< to work with weak_ptrs of objects
     * inheriting from SimpleBGLBase
     */
    template<
        typename T_Graph,
        typename T_SFINAE = typename std::enable_if<
    ...
    std::ostream & operator<<(
        std::ostream & os,
        const std::weak_ptr< T_Graph > graph
    )
    {
    ...
    /**
     * Adapter for operator<< to work with shared_ptrs of objects
     * inheriting from SimpleBGLBase
     */
    template<
        typename T_Graph,
    ...
    operator<<(
        std::ostream & os,
        const std::shared_ptr< T_Graph > graph
    )
    {
        return os << *graph;
    }
