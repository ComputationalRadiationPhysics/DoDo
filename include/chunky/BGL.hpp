...
class BGL {
    ...
    auto consistsOf(VertexID v, VertexID component)
    {
        return boost::add_edge(v, component, (*graph));
    }

    VertexID consistsOf(VertexID v, VertexProperty vp)
    {
        auto newV = addVertex(vp);
        boost::add_edge(v, newV, (*graph));
        return newV;
    }
